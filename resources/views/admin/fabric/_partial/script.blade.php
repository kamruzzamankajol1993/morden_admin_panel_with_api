...
$(document).ready(function() {
    ...
    var routes = {
        fetch: "{{ route('ajax.fabric.data') }}",
        show: id => `{{ route('fabric.show', ':id') }}`.replace(':id', id),
        update: id => `{{ route('fabric.update', ':id') }}`.replace(':id', id),
        delete: id => `{{ route('fabric.destroy', ':id') }}`.replace(':id', id),
        csrf: "{{ csrf_token() }}"
    };
