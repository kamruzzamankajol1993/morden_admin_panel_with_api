...
$(document).ready(function() {
    ...
    var routes = {
        fetch: "{{ route('ajax.color.data') }}",
        show: id => `{{ route('color.show', ':id') }}`.replace(':id', id),
        update: id => `{{ route('color.update', ':id') }}`.replace(':id', id),
        delete: id => `{{ route('color.destroy', ':id') }}`.replace(':id', id),
        csrf: "{{ csrf_token() }}"
    };
    ...
    function fetchData() {
        $.get(routes.fetch, {
            ...
            res.data.forEach((item, i) => {
                ...
                const colorPreview = item.code ? `<div style="width: 30px; height: 30px; background-color: ${item.code}; border: 1px solid #ccc; border-radius: 5px;"></div>` : '';
    ...
    // Link color picker to text input in Add Modal
    $('#addModal').on('input', 'input[type="color"]', function() {
        $(this).prev('input[type="text"]').val($(this).val());
    });

    // Link color picker to text input in Edit Modal
    $('#editModal').on('input', 'input[type="color"]', function() {
        $(this).prev('input[type="text"]').val($(this).val());
    });
    ...
    $(document).on('click', '.btn-edit', function () {
        ...
        $.get(routes.show(id), function (item) {
            ...
            $('#editColorPicker').val(item.code); // Also set the color picker
