...
$(document).ready(function() {
    ...
    var routes = {
        fetch: "{{ route('ajax.unit.data') }}",
        show: id => `{{ route('unit.show', ':id') }}`.replace(':id', id),
        update: id => `{{ route('unit.update', ':id') }}`.replace(':id', id),
        delete: id => `{{ route('unit.destroy', ':id') }}`.replace(':id', id),
        csrf: "{{ csrf_token() }}"
    };
