...
@section('title', 'Add Shareholder Deposit')
@section('css')
    ...
    <div class="container-fluid">
        <div class="card shadow-sm">
            <div class="card-header">
                <h5>Add New Shareholder Deposit</h5>
            </div>
            <div class="card-body">
                @if ($errors->any())
                    <div class="alert alert-danger"><ul>@foreach ($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul></div>
                @endif
                <form action="{{ route('shareholder-deposits.store') }}" method="POST">
                    @csrf
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="cash_account_id" class="form-control select2" required>
                            ...
                            @foreach($cashAccounts as $account)
                                <option value="{{ $account->id }}" @selected(old('cash_account_id') == $account->id)>{{ $account->name }} ({{ $account->code ?? 'N/A' }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3">
                        ...
                        <select name="equity_account_id" class="form-control select2" required>
                            ...
                            @foreach($equityAccounts as $account)
                                <option value="{{ $account->id }}" @selected(old('equity_account_id') == $account->id)>{{ $account->name }} ({{ $account->code ?? 'N/A' }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3">
