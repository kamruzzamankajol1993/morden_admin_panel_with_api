...
@section('title', 'Shareholder Deposits')
@section('body')
<main class="main-content">
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
            <h2 class="mb-0">Shareholder Deposits</h2>
            <div class="d-flex align-items-center">
                <form class="d-flex me-2" role="search">
                    <input class="form-control" id="searchInput" type="search" placeholder="Search by shareholder..." aria-label="Search">
                </form>
                <a href="{{ route('shareholder-deposits.create') }}" class="btn text-white" style="background-color: var(--primary-color);">
                    <i data-feather="plus" class="me-1"></i>Add New Deposit
                </a>
            </div>
        </div>
        <div class="card">
...
$(document).ready(function() {
    ...
    var routes = {
        fetch: "{{ route('ajax.shareholder-deposits.data') }}",
        destroy: id => `{{ url('shareholder-deposits') }}/${id}`,
        csrf: "{{ csrf_token() }}"
    };
    ...
    function fetchData() {
        ...
        $.get(routes.fetch, { page: currentPage, search: searchTerm }, function (res) {
            ...
            if (res.data.length === 0) {
                rows = '<tr><td colspan="6" class="text-center">No deposits found.</td></tr>';
            } else {
                res.data.forEach((deposit, index) => {
                    const editUrl = `{{ url('shareholder-deposits') }}/${deposit.id}/edit`;
                    const depositDate = new Date(deposit.date).toLocaleDateString('en-GB');
                    rows += `<tr>
                        <td>${res.from + index}</td>
                        <td>${deposit.shareholder.name}</td>
                        <td>৳${parseFloat(deposit.amount).toLocaleString('en-IN')}</td>
                        <td>${depositDate}</td>
                        <td>${deposit.note || ''}</td>
                        <td class="text-center">
                            <a href="${editUrl}" class="btn btn-sm btn-warning me-2" title="Edit"><i class="fa fa-edit"></i></a>
                            <button class="btn btn-sm btn-danger btn-delete" data-id="${deposit.id}" title="Delete"><i class="fa fa-trash"></i></button>
                        </td>
                    </tr>`;
                });
            }
