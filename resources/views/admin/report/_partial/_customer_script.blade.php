...
$(document).ready(function() {
    ...
    function fetchData(page = 1, searchQuery = '') {
        ...
        let url = `{{ route('report.customer.data') }}?page=${page}&filter=${currentFilter}`;
    ...
    function renderTable(response) {
        if (response.data.length === 0) {
            tableBody.html('<tr><td colspan="5" class="text-center">No data found for this period.</td></tr>');
            return;
        }
        ...
        response.data.forEach((customer, index) => {
            const row = `
                <tr>
                    ...
                    <td class="text-end"><strong>${new Intl.NumberFormat().format(customer.total_spent)}</strong></td>
                </tr>`;
