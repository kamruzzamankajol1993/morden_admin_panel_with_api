...
$(document).ready(function() {
    ...
    generateBtn.on('click', function() {
        ...
        if (!accountId) {
            Swal.fire('Validation Error', 'Please select an account.', 'warning');
            return;
        }
        ...
        $.ajax({
            url: "{{ route('general_ledger.generate') }}",
            type: 'GET',
            data: {
    ...
    function renderReport(data, accountName) {
        ...
        data.transactions.forEach(entry => {
         ...
         const printUrl = new URL("{{ route('general_ledger.print') }}");
        printUrl.searchParams.append('account_id', accountSelect.val());
        printUrl.searchParams.append('start_date', startDateInput.val());
        printUrl.searchParams.append('end_date', endDateInput.val());

        const reportHtml = `
            <div class="card shadow-sm">
                <div class="card-header d-flex justify-content-between align-items-center no-print">
                    <h5 class="mb-0">Ledger for: ${accountName}</h5>
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td class="text-end fw-bold">${formatCurrency(data.opening_balance)}</td>
                                </tr>
                                ${transactionRows}
