<script>
    ...
    var routes = {
        fetch: "{{ route('ajax.animation_category.data') }}",
        show: id => `{{ route('animationCategory.show', ':id') }}`.replace(':id', id),
        update: id => `{{ route('animationCategory.update', ':id') }}`.replace(':id', id),
        delete: id => `{{ route('animationCategory.destroy', ':id') }}`.replace(':id', id),
        csrf: "{{ csrf_token() }}"
    };
