...
$(document).ready(function() {
    ...
    var routes = {
        fetch: "{{ route('ajax.size.data') }}",
        show: id => `{{ route('size.show', ':id') }}`.replace(':id', id),
        update: id => `{{ route('size.update', ':id') }}`.replace(':id', id),
        delete: id => `{{ route('size.destroy', ':id') }}`.replace(':id', id),
        csrf: "{{ csrf_token() }}"
    };
    ...
    $(document).on('click', '.btn-edit', function () {
        ...
        $.get(routes.show(id), function (item) {
            ...
            $('#editSizeType').val(item.size_type);
