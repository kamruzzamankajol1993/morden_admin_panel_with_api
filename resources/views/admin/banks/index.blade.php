...
<main class="main-content">
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
            <h2 class="mb-0">Bank List</h2>
            <div class="d-flex align-items-center">
                <form class="d-flex me-2" role="search">
                    <input class="form-control" id="searchInput" type="search" placeholder="Search..." aria-label="Search">
                </form>
                <a href="{{ route('banks.create') }}" class="btn text-white" style="background-color: var(--primary-color); white-space: nowrap;">
                    <i data-feather="plus" class="me-1"></i> Add New Bank
                </a>
            </div>
        </div>
        <div class="card">
...
$(document).ready(function() {
    ...
    var routes = {
        fetch: "{{ route('ajax.banks.data') }}",
        destroy: id => `{{ url('banks') }}/${id}`,
        csrf: "{{ csrf_token() }}"
    };
    ...
    function fetchData() {
        ...
        $.get(routes.fetch, { page: currentPage, search: searchTerm }, function (res) {
            ...
            if (res.data.length === 0) {
                rows = '<tr><td colspan="6" class="text-center">No banks found.</td></tr>';
            } else {
                res.data.forEach((bank, index) => {
                    const editUrl = `{{ url('banks') }}/${bank.id}/edit`;
                    const statusBadge = bank.status ? '<span class="badge bg-success">Active</span>' : '<span class="badge bg-danger">Inactive</span>';
                    rows += `<tr>
                        ...
                        <td class="text-center">
                            ...
                            <button class="btn btn-sm btn-danger btn-delete" data-id="${bank.id}" title="Delete"><i class="fa fa-trash"></i></button>
                        </td>
                    </tr>`;
                });
            }
